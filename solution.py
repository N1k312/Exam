...
# "обогащённые" транзакции (revenue, дата, продукт)
...
# 1.7 Session5_Segmentation_and_Recommendations.csv
...
# 1.8 Session5_Product_Performance.csv, Session5_Price_Analysis.csv
def task_1_8_price_optimization(sales, products, out_dir):
    ...
    def suggest_change(ped):
        a = abs(ped)
        if a > 1.0:        # снизить цену
            return "-5%"
        elif a < 1.0 and a > 0:  # поднять цену
            return "+5%"
        return "0%"
...
def main():
    ...
    # Показать, какие столбцы скрипт распознал 
